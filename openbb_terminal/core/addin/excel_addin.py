...
# from xlwings.xlwings.server import serve
import importlib
...
# from xlwings.udfs import call_udf
from . import conversion

from ..library.breadcrumb import Breadcrumb


def OBB(*args):
    trail = args[0].split(".")
    res = getattr(Breadcrumb(trail=".".join(trail[:-1])), trail[-1])(*args[1:])
    res = conversion.write(res, None, {})
    print(res)
    return res
...
def launch_server(clsid="{506e67c3-55b5-48c3-a035-eed5deea7d6d}"):
    ...
    class MyPolicy(BaseDefaultPolicy):
        def _CreateInstance_(self, reqClsid, reqIID):
            if reqClsid == clsid:
                print("Creating XLPython object")
    ...
    msg = "OpenBB Terminal server running, clsid=%s, event loop on %s"
